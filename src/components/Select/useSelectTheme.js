...
export default function useSelectTheme ({ size, variant }) {
  ...
  return function getTheme (selectTheme) {
    const borderRadius = prop(path([size, 'borderRadius'], inputSizes), radii)
    const controlHeight = path([size, 'h'], inputSizes)
    ...
    return {
      ...
      colors: {
        ...
        dangerLight: hexToRgba(colors.palette.red, '0.7'),
